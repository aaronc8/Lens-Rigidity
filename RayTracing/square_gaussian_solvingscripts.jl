## DifferentialEquations practice with objects:
# x0 = 1.0; y0 = -0.5; a0 = -5*pi/6;
# u0 = [x0,y0,cos(a0),sin(a0)];
# prob = ODEProblem(gaussianmetric,u0,(0.0,2.0))
# sol = solve(prob)

# Some notes: It doesn't actually spit out a vector of solutions...?
...
# Somehow, during ODE evolution of a path, the size of the time vector and the solution vector differs...??
...
x = -1:2/Nedge:1;
y = x;
cxy = ones(length(x),length(y));  # exp(0.5.*( x.^2 .+ y'.^2 ));
surf(x,y,cxy)
