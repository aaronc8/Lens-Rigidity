...
## Solve for the derivatives of just c with fft on a square of [p0,pf]^2.
...
## For other rescalings: Input the step size h as well. Then, all the Nedge/4 are just h/2.
# For derivatives, an alternative is to just use the interior points, too.

Nedge = length(cxy[:,1]) - 1;   # So h = 2/Nedge;
gradcxy = zeros(Nedge+1,Nedge+1,2);
hesscxy = zeros(Nedge+1,Nedge+1,2,2);

k = 2:Nedge;  # Skips endpoints
...
function hesscspd(x,y)
  hess = zeros(2,2);  # Or, Array{Array}(2,2) is another perspective, but didn't work for "single" values.
  hess[1,1] = d2cdx2[x,y];
  hess[1,2] = d2cdxy[x,y];
  hess[2,1] = d2cdyx[x,y];
  hess[2,2] = d2cdy2[x,y];
...
while isempty(k) && maxiter < 1000*scaling ## In case of trapped rays
    ...
    kshift = length(u);
    s = append!(s,sadd[2:end]);   # Because the 1st entry is the initial data.
    u = append!(u,uadd[2:end]);
    maxiter = maxiter + 1;
end
...
# Construct the row in the system solving for the update.
# These entries' weights should sum to zero for the derivative approx.
...
s = 0.0:tf/2.0^4:tf;
h = tf/2.0^4; n = 2.0^4;   # This way 2^(-16) ~ 1e-5 error with Simpson.
...
# an angle of incidence.
  ...
  for i = 1:Nedge-1
      ...
      for j = 1:Nangle-1
          ...
          RayData = linearMismatch(JE,XE,sEout,cn,gradcn,Nedge);
          ...
          b[4*(rowcount-1)+1:4*rowcount] = (XE(sEout) - bexact[4*(rowcount-1)+1:4*rowcount])./L2err;
          rowcount = rowcount + 1;
      end
  end
  for i = 1:Nedge-1
      ...
      for j = 1:Nangle-1
          ...
          RayData = linearMismatch(JN,XN,sNout,cn,gradcn,Nedge);
          ...
          b[4*(rowcount-1)+1:4*rowcount] = (XN(sNout) - bexact[4*(rowcount-1)+1:4*rowcount])./L2err;
          rowcount = rowcount + 1;
      end
    end
    for i = 1:Nedge-1
        ...
        for j = 1:Nangle-1
          ...
          RayData = linearMismatch(JW,XW,sWout,cn,gradcn,Nedge);
          ...
          b[4*(rowcount-1)+1:4*rowcount] = (XW(sWout) - bexact[4*(rowcount-1)+1:4*rowcount])./L2err;
          rowcount = rowcount + 1;
        end
    end
    for i = 1:Nedge-1
        ...
        for j = 1:Nangle-1
          ...
          RayData = linearMismatch(JS,XS,sSout,cn,gradcn,Nedge);
          ...
          b[4*(rowcount-1)+1:4*rowcount] = (XS(sSout) - bexact[4*(rowcount-1)+1:4*rowcount])./L2err;
          rowcount = rowcount + 1;
      end
  end
